...
- (void)loadUI
{
    ...
    NSArray *titleAry = [NSArray arrayWithObjects:@"人人网", @"陌陌", @"微博", @"BBS", @"微信", nil];
    int count = (int)[titleAry count];
    for (int i = 0; i < count; i ++)
    {
        LmcItemsView *item = [[LmcItemsView alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
        item.tag = 1000 + i;
        item.startPoint = EndTPOINT;
//        item.endPoint = CGPointMake(EndTPOINT.x - ENDRADIUS * sinf(i * M_PI_2 / (count - 1)), EndTPOINT.y - ENDRADIUS * cosf(i * M_PI_2 / (count - 1)));
        item.endPoint = CGPointMake(EndTPOINT.x - ENDRADIUS * sinf(i * M_PI_2 / (count - 1)), EndTPOINT.y - (ENDRADIUS + i *20) * cosf(i * M_PI_2 / (count - 1)));
